...
add_action( 'wpcf7_init', 'cellep_wpcf7_add_shortcode_submit' );

function cellep_wpcf7_add_shortcode_submit() {
	wpcf7_add_shortcode( 'submit', 'cellep_wpcf7_submit_shortcode_handler' );
}

function cellep_wpcf7_submit_shortcode_handler( $tag ) {
...
if ( is_admin() ) {
	add_action( 'admin_init', 'cellep_wpcf7_add_tag_generator_submit', 55 );
}

function cellep_wpcf7_add_tag_generator_submit() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'submit', __( 'submit', 'contact-form-7' ),
		'cellep_wpcf7_tag_generator_submit', array( 'nameless' => 1 ) );
}

function cellep_wpcf7_tag_generator_submit( $contact_form, $args = '' ) {
